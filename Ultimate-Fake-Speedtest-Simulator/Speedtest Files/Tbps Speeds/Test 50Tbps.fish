function speedtest
    ...
    echo -e "\033[1;36m[SYSTEM]\033[0m Initializing Regional Terabit Switching Fabric..."
    sleep 1
    ...
    echo -ne "\033[1;33m[SCAN]\033[0m  Aligning Terabit Lambda Channels..."
    sleep 3 
    
    echo -e "\r\033[1;32m[FOUND]\033[0m Local Node: Mogadishu-BACKBONE-01 (Latency: "$base_pg"ms)         "
    echo -ne "\033[1;33m[QoS]\033[0m   Calibrating Coherent Optics..."
    sleep 1.2
    echo -e "\r\033[1;32m[OK]\033[0m    Line Status: \033[1;32mOPTIMAL\033[0m (50T Path Verified)"
    sleep 1
    echo ""
    ...
    echo -e "Interface:[\033[1;32m DWDM-BACKBONE \033[0m] | Status:      [\033[1;32m 50TBPS ACTIVE \033[0m]"
    ...
    # --- PHASE 3 & 4: Ramp-Up (Climbing to 50,000,000 Mbps) ---
    for i in (seq 1 20)
        # Target is 50,000,000 Mbps
        set dl (math "2500000 * $i")
        set ul (math "2400000 * $i")
    ...
    # --- PHASE 5: Infinite Live Monitor (50Tbps Zone) ---
    while true
        set pg (random 0 1)
        set jt_raw (random 1 2)
        set jt (math -s1 "$jt_raw / 10")

        # 50Tbps Speeds (47,000,000 to 49,850,000 Mbps)
        set dl (math (random 47200000 49850000) + (random 1 9)/10) 
        set ul (math (random 45150000 48480000) + (random 1 9)/10)
        
        # Data Accumulation: 50Tbps is 6,250GB/s. Loop 0.4s = 2,500GB (2,560,000MB)
        set total_data (math "$total_data + 2560000")
