function speedtest
    ...
    # Discovery Pings (Sub-millisecond for Tier-1 Carriers)
    set lon_pg (random 1 3)    
    set ger_pg (random 1 3)   
    set fra_pg (random 2 5)   
    set dxb_pg (random 10 20)  
    ...
    sleep 1.2
    echo -e "\r\033[1;32m[OK]\033[0m    Line Status: \033[1;32mOPTIMAL\033[0m (QSFP-DD Path Clear)"
    sleep 1
    echo ""
    ...
    echo -e "Interface:[\033[1;32m 100G QSFP  \033[0m] | Status:      [\033[1;32m 100GBPS CORE  \033[0m]"
    ...
    # --- PHASE 3 & 4: Ramp-Up (Climbing to 100,000 Mbps) ---
    for i in (seq 1 20)
        # Target is 100,000 Mbps
        set dl (math "5000 * $i")
        set ul (math "4800 * $i")
        
        # Zero-latency core sync
        set pg (random 0 1)
        set jt_raw (random 1 2)
        set jt (math -s1 "$jt_raw / 10")

        echo -ne "\r\033[1;35mPing: $pg ms\033[0m | \033[1;33mJitter: $jt ms\033[0m | \033[1;32mDL: $dl Mbps\033[0m | \033[1;34mUL: $ul Mbps\033[0m | \033[1;32mMODE: SYNC\033[0m \n\033[1;32mTraffic: [+] \033[1;37mSent: 0GB\033[0m"
        echo -ne "\033[1A"
        sleep 0.1
    end

    # --- PHASE 5: Infinite Live Monitor (100Gbps Zone) ---
    while true
        ...
        # 100Gbps Speeds (94,000 to 99,500 Mbps)
        set dl (math (random 94200 99500) + (random 1 9)/10) 
        set ul (math (random 91500 96800) + (random 1 9)/10)
        
        # Data Accumulation: 100Gbps is 12.5GB/s. Loop 0.4s = 5GB (5120MB)
        set total_data (math "$total_data + 5120")
