function speedtest
    ...
    # Discovery Pings
    set lon_pg (random 1 9)    
    set ger_pg (random 1 9)    
    set fra_pg (random 5 15)   
    set dxb_pg (random 25 35)  
    ...
    echo -e "Streaming:[\033[1;32m EXCELLENT \033[0m] | Status:      [\033[1;32m 10GBPS ACTIVE \033[0m]"
    ...
    for i in (seq 1 30)
        set dl (math "min(9900, 350 * $i)")
        set ul (math "min(9500, 320 * $i)")
        
        # Dynamic Latency during test
        set pg (random 1 3)
        set jt_raw (random 1 5)
    ...
    # --- PHASE 5: Infinite Live Monitor (Dynamic 10Gbps Zone) ---
    while true
        # Real-time Latency Jumps
        set pg (random 1 4)
        set jt_raw (random 2 8)
        set jt (math -s1 "$jt_raw / 10")

        # 10Gbps Speeds
        set dl (math (random 9420 9890) + (random 1 9)/10) 
        set ul (math (random 9150 9480) + (random 1 9)/10)
        
        # Data Accumulation (10Gbps moves ~1.25GB/s)
        set total_data (math "$total_data + 512")
