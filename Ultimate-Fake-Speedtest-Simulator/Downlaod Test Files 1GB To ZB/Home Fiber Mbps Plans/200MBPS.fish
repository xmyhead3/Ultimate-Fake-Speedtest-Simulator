function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 200Mbps Ultra-Fiber...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 200MBPS ULTRA-FIBER ACTIVE \033[0m]\n"
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            set random_mbps (random 195 205)
            set random_ms (random 5 10)
        end

        # MB per tick (Mbps / 8 bits / 10 ticks per second)
