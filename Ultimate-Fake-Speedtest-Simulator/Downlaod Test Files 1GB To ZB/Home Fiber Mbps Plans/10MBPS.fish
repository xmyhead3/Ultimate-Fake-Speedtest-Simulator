function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 10Mbps Plus Fiber...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 10MBPS PLUS FIBER ACTIVE \033[0m]\n"
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            # Fixed the math error by pulling random out of the string
            set -l raw_rand (random 92 108)
            set random_mbps (math -s1 "$raw_rand / 10") 
            set random_ms (random 35 42)
        end
