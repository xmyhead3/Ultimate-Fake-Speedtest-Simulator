function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 5Mbps Starter Fiber...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 5MBPS STARTER FIBER ACTIVE \033[0m]\n"
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            # FIX: Calculate random raw value first to avoid math error
            set -l raw_rand (random 47 53)
            set random_mbps (math -s1 "$raw_rand / 10") 
            set random_ms (random 42 50)
        end
        ...
        set bar_count (math -s0 "$percent/4")
        set bar (string repeat -n $bar_count "█")
        set empty_count (math "25-$bar_count")
        set empty (string repeat -n $empty_count "░")
        ...
        if test $current_mb -ge 1125899906842624; set uc "ZB"; set sc (math -s2 "$current_mb/1125899906842624")
        else if test $current_mb -ge 1099511627776; set uc "EB"; set sc (math -s2 "$current_mb/1099511627776")
        else if test $current_mb -ge 1073741824; set uc "PB"; set sc (math -s2 "$current_mb/1073741824")
        else if test $current_mb -ge 1048576; set uc "TB"; set sc (math -s2 "$current_mb/1048576")
        else if test $current_mb -ge 1024; set uc "GB"; set sc (math -s1 "$current_mb/1024")
        else; set uc "MB"; set sc (math -s1 "$current_mb"); end

        printf "\r%3s%% [%-25s] %.2f%s / %.2f%s | Latency: \033[1;36m%dms\033[0m | Speed: \033[1;32m%s Mbps\033[0m | ETA: %s    " \
