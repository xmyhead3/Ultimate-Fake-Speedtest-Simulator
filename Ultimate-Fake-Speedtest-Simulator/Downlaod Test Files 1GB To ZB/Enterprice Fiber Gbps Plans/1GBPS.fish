function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 1Gbps Fiber Environment...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 1GBPS LINK ACTIVE \033[0m]\n"
    ...
    set random_mbps 950  # Initial 1Gbps value
    set random_ms 12
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            if test (random 1 50) -eq 1
                set random_mbps 15
                set random_ms (random 150 300)
                set next_shift_ticks (math $tick_counter + 60) 
            else
                set random_mbps (random 850 1020) # Fluctuation for 1Gbps
                set random_ms (random 10 18)
            end
        end
