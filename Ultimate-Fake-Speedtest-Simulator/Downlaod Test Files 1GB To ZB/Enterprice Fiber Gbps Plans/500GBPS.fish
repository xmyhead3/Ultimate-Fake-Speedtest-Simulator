function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 500Gbps Backbone Core...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 500GBPS BACKBONE ACTIVE \033[0m]\n"
    ...
    set random_mbps 500000  # 500Gbps = 500,000 Mbps
    set random_ms 1
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            set random_mbps (random 485000 515000) 
            set random_ms (random 1 3)
        end

        # Calc MB per 0.1s tick
