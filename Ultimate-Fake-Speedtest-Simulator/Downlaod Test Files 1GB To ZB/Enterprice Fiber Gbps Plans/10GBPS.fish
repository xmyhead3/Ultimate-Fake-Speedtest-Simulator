function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 10Gbps Fiber Environment...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 10GBPS LINK ACTIVE \033[0m]\n"
    ...
    set random_mbps 9800  # Initial 10Gbps value
    set random_ms 5
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            if test (random 1 50) -eq 1
                set random_mbps 20
                set random_ms (random 120 250)
                set next_shift_ticks (math $tick_counter + 60) 
            else
                set random_mbps (random 9200 10500) # Fluctuation for 10Gbps
                set random_ms (random 4 8)
            end
        end
        ...
        # --- ETA LOGIC (Days instead of Months) ---
