function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 100Pbps Exascale-Grid...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 100PBPS EXASCALE LINK ACTIVE \033[0m]\n"
    ...
    set random_mbps 100000000000  # 100Pbps = 100 Billion Mbps
    set random_ms 0
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            set random_mbps (random 99000000000 101000000000) 
            set random_ms 0
        end
        ...
        printf "\r%3s%% [%-25s] %.2f%s / %.2f%s | Latency: \033[1;36m%dms\033[0m | Speed: \033[1;32m%s Mbps\033[0m" \
            $percent "$bar$empty" $sc $uc $st $ut $random_ms $speed_fmt
    ...
    printf "\n\n\033[1;32m✔ 100Pbps Exascale Data Injection Complete\n✔ Quantum Parity Shield Verified\n[FINISH]\033[0m Processed %s in %s sec.\n" $size_name $total_time
end