function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 50Pbps Interstellar-Core Array...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 50PBPS INTERSTELLAR ACTIVE \033[0m]\n"
    ...
    set random_mbps 50000000000  # 50Pbps = 50 Billion Mbps
    set random_ms 0
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            # Intense throughput fluctuation
            set random_mbps (random 49000000000 51500000000) 
            set random_ms 0
        end

        # Speed logic: bits to bytes (8) over 10 ticks per sec
    ...
    printf "\n\n\033[1;32m✔ 50Pbps Hyper-Speed Transfer Successful\n✔ SHA-1024 Quantum Checksum Verified\n[FINISH]\033[0m Processed %s in %s sec.\n" $size_name $total_time
end