function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 1Tbps Backbone Environment...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 1TBPS BACKBONE ACTIVE \033[0m]\n"
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            if test (random 1 100) -eq 1
                set random_mbps 500
                set random_ms 45
                set next_shift_ticks (math $tick_counter + 30) 
            else
                set random_mbps (random 975000 1025000)
                set random_ms 0
            end
        end
