function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 50Tbps Subsea-Link Array...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 50TBPS SUBSEA-LINK ACTIVE \033[0m]\n"
    ...
    set random_mbps 50000000  # 50Tbps base in Mbps
    set random_ms 0
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            # Throughput fluctuation around 50Tbps
            set random_mbps (random 48000000 52000000) 
            set random_ms (random 0 1)
        end

        # Speed divided by 8 (bits to bytes) divided by 10 (ticks per sec)
