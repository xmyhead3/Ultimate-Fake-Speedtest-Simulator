function download_test
    ...
    printf "\033[1;36m[STORAGE]\033[0m Initializing Hormuud 10Tbps Regional Fiber Trunk...\n\n"
    ...
    printf "Resource:  %s\n" $random_res
    printf "Node:      %s\n" $random_node
    printf "Status:    [\033[1;32m 10TBPS REGIONAL-TRUNK ACTIVE \033[0m]\n"
    ...
    set random_mbps 10000000  # 10Tbps base in Mbps
    set random_ms 1
    ...
    while test $current_mb -lt $total_mb
        if test $tick_counter -ge $next_shift_ticks
            ...
            # Throughput fluctuation around 10Tbps
            set random_mbps (random 9800000 10200000) 
            set random_ms (random 1 2)
        end

        # Speed divided by 8 (bits to bytes) divided by 10 (ticks/sec)
        set speed_mb (math -s0 "$random_mbps / 8 / 10")
        ...
        # --- CORRECTED UNIT SCALING (RE-BUILT FOR ACCURACY) ---
        # 1GB = 1,024 MB | 1TB = 1,048,576 MB | 1PB = 1,073,741,824 MB
        ...
        else if test $total_mb -ge 1024; set st (math -s2 "$total_mb/1024"); set ut "GB"
        ...
        else if test $current_mb -ge 1024; set sc (math -s2 "$current_mb/1024"); set uc "GB"
        else; set sc (math -s0 "$current_mb"); set uc "MB"; end

        set speed_fmt (printf "%'d" $random_mbps)


        printf "\r%3s%% [%-25s] %.2f%s / %.2f%s | Latency: \033[1;36m%dms\033[0m | Speed: \033[1;32m%s Mbps\033[0m | ETA: %s    " \
            $percent "$bar$empty" $sc $uc $st $ut $random_ms $speed_fmt $etad
